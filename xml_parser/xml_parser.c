...
static void resize_cell(mxArray * mx, mwSize N) {
  mwIndex i;
  size_t n = mxGetNumberOfElements(mx);
  mxSetData(mx, mxRealloc(mxGetData(mx), N  * sizeof(mxArray *)));
  mxSetN(mx, N);
  for(i = n; i < N; i++)
    mxSetCell(mx, i,NULL);
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  char *XML_CURR = NULL;
  /*
  const char **nodefields = (const char *[]){"type", "tag", "attributes", "children", "id", "parent","attribute_keys"};
  const char **attrfields = (const char *[]){"key", "value"};
   */
  const char **nodefields = mxMalloc(sizeof(char*) * 7);
  ...
  nodefields[4] = "id";
  nodefields[5] = "parent";
  nodefields[6] = "attribute_keys";
  attrfields[0] = "key";
  attrfields[1] = "value";
  
  /* Initialisation */
  plhs[0] = mxCreateStructMatrix(1, 0, 7, nodefields);
  ...
  for(doc = xml; *doc; doc++) {
    ...
    switch (r) {
      ...
      case YXML_ELEMEND:
        ...
        switch (r) {
          case YXML_ELEMSTART:
            ...
            mxSetFieldByNumber(plhs[0],XML_UID-1,6,mxCreateCellMatrix(1,1));
      ...
      case YXML_ATTRSTART:
        ...
        resize_cell(mxGetFieldByNumber(plhs[0],XML_UID-1,6),nattr);
        mxSetCell(mxGetFieldByNumber(plhs[0],XML_UID-1,6),nattr-1,mxCreateString(x->attr));
        break;
      case YXML_ATTRVAL:
